 ...
 ;(function ($, window, document, undefined) {
  ...
  function Plugin(element, options) {
    ...
    this.periodOptions = {
      ...
      currentPeriod: this.settings.currentPeriod,
    };
    this.init();
  }
  ...
  $.extend(Plugin.prototype, {
    ...
    init: function () {
      ...
      this.periodInterval = 60 / this.settings.periodDuration; //Cantidad de bloques en que se dividirá la hora de clase
      this.periodHeight = 30 * this.periodInterval; // Altura del recuadro dentro de la franja de hora
      this.periodPosition = 26.7 / this.periodInterval; //Posicion del recuadro dentro de la franja de hora
    ...
    create: function () {
      ...
      for (var j = 0; j < 25; j++) {
        $('<div class="jqs-grid-line" id="FranjaHora'+j+'"><div class="jqs-grid-hour">' + this.formatHour(j) + '</div></div>').
    ...
    add: function (parent, position, height, options) {
      ...
      var periodTitle = '<div class="jqs-period-title"><br>' + options.title + '<br></div>';
      var periodTime = '<div class="jqs-period-time">' + this.periodInit(position, position + height) + '&#10</div>';
      var period = $('<div class="jqs-period">' +
        '<div class="jqs-period-container">' + periodTime + periodTitle + periodRemove + periodDuplicate + '</div>' +
        '</div>').css({
        'top': position * this.periodPosition,
        'height': height * this.periodPosition
      }).attr('id', 'periodoAsignatura').attr('title', options.title).appendTo(parent);
      //}).attr('id', this.uniqId()+'_upok').attr('title', options.title).appendTo(parent); <-- LINEA DE RESPALDO
